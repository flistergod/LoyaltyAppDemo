...
<html lang="en">
  ...
  <div class="row mr-0">
    <div class="col-4 mt-5 ml-3">
        <p class="frase"> Transações </p>
    </div>
  </div>
<div class="table-responsive">
<table class="table table-bordered table-striped w-50 ml-auto mr-auto mt-5">
  <thead class="black white-text">
   ...
   <?php 
        ...
        if($transacoes->num_rows()>0){
            foreach($transacoes->result() as $row){

        ?>
  <tbody>
    <tr>
      <th scope="row"><?php echo $row->ID_TRANSACAO; ?></th>
      <td><?php echo $row->DESCRICAO_TRANSACAO; ?></td>
...
<div class="col-7 mt-5 ml-3">
        <a href="<?php echo site_url('user/transacao1/'.$row->ID) ?>"><button type="button" class="btn btn-danger float-right mt-2"> Inserir Transação</button></a>
    </div>
...
<footer class="page-footer fixed-bottom font-small bg-light">
    ...
    <script type="text/javascript">
         /*$(document).ready(function(){
            $('#apagarCamp').click(function(){
              //  var id = $(this).attr("NOME");
                if (confirm("Are you sure you want to delete this?")) {
                        window.location ="<?php echo site_url('campanha_ctl/delete_Camp/'.$row->ID_CAMPANHA) ?>";
                }else{
                        return false;
                }
            })
